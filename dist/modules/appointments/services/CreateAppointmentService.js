...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const startedDate = (0, _dateFns.startOfHour)(date);

    if ((0, _dateFns.isBefore)(startedDate, Date.now())) {
      throw new _AppError.default("You can't create an appointment on a past date.");
    } // throw new AppError("You can't create an appointment on a past date.");


    if (user_id === provider_id) {
      throw new _AppError.default("You can't create an appotment with yourself");
    }

    const hour = (0, _dateFns.getHours)(startedDate);

    if (hour > 17 || hour < 8) {
      throw new _AppError.default('You can only create appotments between 8am and 5pm');
    }

    const findAppointmentInSameDate = await this.appointmentsRepository.findByDate(startedDate, provider_id);
    ...
    const dateFormated = (0, _dateFns.format)(startedDate, "dd/MM/yyyy 'às' HH:mm'h'");
    await this.notificationRepository.create({
      recipient_id: provider_id,
      content: `Novo Agendamento para dia ${dateFormated}`
    });
    await this.cacheProvider.invalidate(`provider-appointments:${provider_id}:${(0, _dateFns.format)(startedDate, 'yyyy-M-d')}`);
    return appointment;
  }

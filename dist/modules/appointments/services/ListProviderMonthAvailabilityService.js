...
let ListProviderMonthAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    }, (_, index) => index + 1); // console.log(eachDaysArray);

    const availability = eachDaysArray.map(day => {
      const compareDate = new Date(year, month - 1, day, 23, 59, 59);
      const appointmentInDay = appointments.filter(appontment => {
        return (0, _dateFns.getDate)(appontment.date) === day;
      });
      return {
        day,
        available: (0, _dateFns.isAfter)(compareDate, new Date()) && appointmentInDay.length < 10
      };
    });
    return availability;
  }
